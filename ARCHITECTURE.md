...
**The tick orchestrator has no direct tests.** Every component it wires is well covered, several with real subprocess and real-git integration tests, but `src/index.ts` itself, the file that decides the *order* those gates run in, is verified only through its parts. A sequencing regression, a gate checked too late or against stale data, is the highest-blast-radius bug available here and the one class the current suite would not catch.
